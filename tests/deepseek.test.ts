...
import { DeepSeekReportWriter, REPORT_SYSTEM_PROMPT } from "@/lib/server/deepseek-writer";
...
describe("DeepSeek report writer", () => {
  ...
  it.each(["malformed", "truncated", "server", "empty"])("fails closed after two %s responses", async failure => {
    ...
    await expect(new DeepSeekReportWriter("test-only", undefined, request).write(f.input)).rejects.toThrow("DeepSeek request failed");
    expect(request).toHaveBeenCalledTimes(2);
  });
