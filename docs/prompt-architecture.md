...
- 聊天结束之后的空白不是证据：导出的聊天不带导出日期，她可能几天前就导出了。模型只拿到聊天结束的日期（`chatEnds`），不再拿到"离今天几天"；把结束之后说成"他再没联系""沉默了几天"的句子会被删掉，聊天里面的空白照常可以写。
- "最近 N 周"的统计：聊天在最后一个自然周的前三天就结束时，这一周不算作最近一周，免得只有一天的周把次数算少。
