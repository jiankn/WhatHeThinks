...
- 新批677词返回量（676个正值、1个为0），323词未返回量；两批合计1267词有量。不要将近似变体的量相加。
- 哥飞KD：成功精评20词；保存原始分、常规分、竞争页面、原因与链接预算。未force重算。
...
|---|---|---:|---:|---|
| P0 | text message analyzer | 110 | 0 | 首页主词 `/` |
| P0 | how to tell if a guy likes you over text | 480 | 15.6 | 升级现有页 `/does-he-like-me-text-analyzer` |
| P0 | love bombing vs genuine interest | 590 | 18 | 新对比页 `/love-bombing-vs-genuine-interest` |
| P0 | situationship vs relationship | 1000 | 19.5 | 新对比页 `/situationship-vs-relationship` |
| P0 | what does this text mean | 880 | 24.2 | 首页辅助意图 `/` |
| P0 | relationship check in questions | 1300 | 28.6 | 新实用清单 `/relationship-check-in-questions` |
| P0 | dry texting | 3600 | 29.4 | 新内容页 `/dry-texting` |
| P0 | signs he is losing interest | 480 | 30.4 | 升级现有页 `/is-he-losing-interest` |
| P0 | texting after first date | 590 | 31.3 | 新情境指南 `/texting-after-first-date` |
| P0 | should i text my ex | 1600 | 32.9 | 新决策指南 `/should-i-text-my-ex` |
| P1 | ghosting after first date | 390 | 30.5 | 后续情境指南 `/ghosting-after-first-date` |
| P1 | long distance relationship communication | 1000 | 37.7 | 后续指南 `/long-distance-relationship-communication` |
| P1 | breadcrumbing examples | 720 | 41.4 | 扩充现有页 `/breadcrumbing-test` |
| P1 | bare minimum in a relationship | 1000 | 47.6 | 后续指南 `/bare-minimum-in-a-relationship` |
| P1 | one sided relationship | 2400 | 49.3 | 后续专题 `/one-sided-relationship` |
| P1 | talking stage | 5400 | 52.2 | 后续主题入口 `/talking-stage` |
| P2 | soft ghosting | 260 | 49 | 延后 `/soft-ghosting` |
| P2 | double texting | 2900 | 54.4 | 延后 `/double-texting` |
| P2 | future faking | 4400 | 63.9 | 延后 `/future-faking` |
| P2 | export whatsapp chat | 720 | 64.1 | 帮助内容优先于SEO投入 `/export-whatsapp-chat` |
...
- 页面：`/`；转化入口：`/analyze`。
- 内容要求：量仅110但产品意图强，哥飞盘面有低DR工具站；首页可扩展到此主词，relationship text analyzer 作为精准同义辅助。KD=0不等于没有竞争。
### how to tell if a guy likes you over text

- 页面：`/does-he-like-me-text-analyzer`；转化入口：`/analyze`。
- 内容要求：升级主词而不是另建重复页；用具体聊天例子先回答，再展示分析器。原 does he like me over text 保留为同页辅助词。
### love bombing vs genuine interest

- 页面：`/love-bombing-vs-genuine-interest`；转化入口：`/mixed-signals-text-analyzer`。
- 内容要求：围绕节奏、边界、持续行动和具体聊天例子，不做人格诊断，也不宣称算法能判定操纵意图。
### situationship vs relationship

- 页面：`/situationship-vs-relationship`；转化入口：`/situationship-analyzer`。
- 内容要求：对比承诺、排他性、计划、沟通边界，附自查清单；与分析器区分“概念比较”和“分析我的聊天”。
### what does this text mean

- 页面：`/`；转化入口：`/analyze`。
- 内容要求：880/月但含缩写、图标等歧义；作为首页的真实问题入口，明确需要关系语境；不另做同功能解释器页。
### relationship check in questions

- 页面：`/relationship-check-in-questions`；转化入口：`/analyze`。
- 内容要求：按投入、期待、联系节奏和计划提供可直接使用的问题；可做可复制清单，再用报告帮助准备谈话。
### dry texting

- 页面：`/dry-texting`；转化入口：`/is-he-losing-interest`。
- 内容要求：定义、自然聊天例子、忙碌与持续冷淡的区别；演示回复长度和主动比例如何一起变化。不要只堆几十条模板回复。
### signs he is losing interest

- 页面：`/is-he-losing-interest`；转化入口：`/analyze`。
- 内容要求：480/月词作为主要内容目标；保留原问题和 URL，解释多周变化及反例，不凭单次慢回复下结论。
### texting after first date

- 页面：`/texting-after-first-date`；转化入口：`/does-he-like-me-text-analyzer`。
- 内容要求：何时联系、消息例子、对方未回复怎么办；不要使用机械的等几小时规则，也别复制兴趣判断页。
### should i text my ex

- 页面：`/should-i-text-my-ex`；转化入口：`/ex-text-analyzer`。
- 内容要求：按联系目的、对方边界和过往互动分支给建议；与前任是否想复合页面区分意图，不保证复合。
