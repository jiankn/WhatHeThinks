...
- 报告先用 DeepSeek（`DEEPSEEK_API_KEY`）写，不合格再换智谱 GLM（`FALLBACK_API_KEY`，模型见 `wrangler.jsonc` 的 `GLM_MODEL`）。两个都没配时阻止新结账，不生成模板报告。失败原因记在 `events` 表（`generate_failed` / `generate_recovered`，后者含换模型和自动修复）。
