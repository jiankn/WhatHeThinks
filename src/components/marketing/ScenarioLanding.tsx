...
export function ScenarioLanding({ page }: { page?: LandingPage }) {
  ...
  return <main className="v3-marketing">
    <section className="v3-hero">
      ...
      <p>{page?.lede ?? <>An AI text message analyzer for your relationship conversations. Upload a WhatsApp export to our <Link href="/whatsapp-relationship-analyzer" className="v3-inline-link">WhatsApp chat analyzer</Link> or paste messages to explore effort, mixed signals and changes over time.</>}</p>
    ...
    {page && <section className="v3-section v3-scenario-details"><header className="v3-section-heading"><h2>What this report looks at</h2></header><dl>{page.measures.map(m => <div key={m.name}><dt>{m.name}</dt><dd>{m.how}</dd></div>)}</dl>{page.answers.map(a => <details key={a.q}><summary>{a.q}</summary><p><LinkedText text={a.a} /></p>{a.list && <ul>{a.list.map(x => <li key={x}>{x}</li>)}</ul>}</details>)}</section>}
