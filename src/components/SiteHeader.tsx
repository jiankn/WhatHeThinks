...
export function SiteHeader() {
  ...
  return <header className="v3-header"><Logo /><nav aria-label="Main navigation">{setup ? <Link href="/" aria-label="Close setup" className="v3-close">×</Link> : <><Link href="/guides">Guides</Link><Link href="/sample-report" className="v3-nav-sample">Sample report</Link><Link href="/account" className="v3-nav-account">Account</Link><Link href="/analyze" className="v3-nav-start">Try it now <span aria-hidden="true">↗</span></Link></>}</nav></header>;
}
