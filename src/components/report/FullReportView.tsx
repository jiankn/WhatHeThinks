...
export function FullReportView({ view, report, token, onDelete, deleteBusy = false, deleteError = null, sample = false }: {
  ...
  return <main className="reader-shell" lang="en">
    ...
    {report.story ? <StorySections story={report.story} report={report} liteMode={view.preview.liteMode} evidence={view.evidence ?? []} value={value} open={open} /> : report.narrative ? <NarrativeSections narrative={report.narrative} report={report} liteMode={view.preview.liteMode} evidence={view.evidence ?? []} claims={claims} value={value} open={open} /> : <>
    ...
    <ShareResult preview={view.preview} reportId={sample ? undefined : view.id} token={token} sample={sample} reportHeadline={report.story || report.narrative ? report.summary.headline : undefined} />
