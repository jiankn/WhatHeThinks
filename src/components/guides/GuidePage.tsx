...
export function GuidePage({ guide }: { guide: Guide }) {
  ...
  return <main className="guide-shell">
    ...
    <header className="guide-header"><p className="guide-kicker">{guide.category}</p><h1>{guide.title}</h1><p className="guide-intro">{guide.intro}</p><p className="guide-byline">By WhatHeThinks · Updated <time dateTime={GUIDES_UPDATED}>September 22, 2026</time></p></header>
    <div className="guide-layout">
      ...
      <article className="guide-body">
        ...
        {guide.sections.map((s, i) => <section key={s.heading} id={`section-${i + 1}`}><h2>{s.heading}</h2>{s.paragraphs.map(p => <p key={p}>{p}</p>)}
        ...
        <section id="questions"><h2>Common questions</h2>{guide.faq.map(f => <details key={f.q}><summary>{f.q}</summary><p>{f.a}</p></details>)}</section>
